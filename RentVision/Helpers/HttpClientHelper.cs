...
using Twinvision.Piranha.RentVision.Models.Interfaces;
...
namespace Twinvision.Piranha.RentVision.Helpers
{
    public class HttpClientHelper<T> : IHttpClientHelper<T>
    {
        ...
        public async Task<T> GetSingleItemRequest(string apiUrl, CancellationToken cancellationToken)
        {
            ...
            var response = await client.GetAsync(BackOffice.Url + apiUrl, cancellationToken).ConfigureAwait(false);
        ...
        public async Task<T> PostRequest(string apiUrl, T postObject, CancellationToken cancellationToken)
        {
            T result = default(T);
            var client = _clientFactory.CreateClient("RentVisionApi");
            var postObjectSerialized = new StringContent(JsonConvert.SerializeObject(postObject), Encoding.UTF8, "application/json");
            var response = await client.PostAsync(BackOffice.Url + apiUrl, postObjectSerialized, cancellationToken).ConfigureAwait(false);
            if (response.IsSuccessStatusCode)
            {
                await response.Content.ReadAsStringAsync().ContinueWith((Task<string> x) =>
                {
        ...
        {
            var client = _clientFactory.CreateClient("RentVisionApi");
            var putObjectSerialized = new StringContent(JsonConvert.SerializeObject(putObject), Encoding.UTF8, "application/json");
            var response = await client.PutAsync(BackOffice.Url + apiUrl, putObjectSerialized, cancellationToken).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
            {
                var content = await response.Content.ReadAsStringAsync();
                response.Content?.Dispose();
        ...
        {
            var client = _clientFactory.CreateClient("RentVisionApi");
            var response = await client.DeleteAsync(BackOffice.Url + apiUrl, cancellationToken).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
            {
                var content = await response.Content.ReadAsStringAsync();
                response.Content?.Dispose();
