...
function handleHeaderScroll(e)
{
	...
	$("header").css("background", "#222").css("border-bottom", "1px solid #222");
	$("header .headerOption").css("background", "rgba(255,255,255,1)").css("color", "#000000").css("border-bottom", "#EEE");

	if ( top <= 70 )
	{
		$("header").css("background", "rgba(0,0,0,0)").css("border-bottom", "none");
        $("header .headerOption").css("background", "rgba(0,0,0,0.0)").css("color", "#FFFFFF").css("border-bottom", "rgba(255,255,255,0)");
...
// Events
$(document).ready(init);
$("body").on("scroll", handleHeaderScroll);
handleHeaderScroll();